...
*/
TFL_RESULT  TFL_Common::DepthToPCD(uint16_t depthBuf[TFL_FRAME_SIZE], TFL_PointXYZ pcdBuf[TFL_FRAME_SIZE])
{	
	...
	{
		pcdBuf->x = (float)(*depthBuf * __x_cal[i]);
		pcdBuf->y = (float)(*depthBuf * __y_cal[i]);
		pcdBuf->z = (float)(*depthBuf * __z_cal[i]);
